$(document).ready(function () {
    ...
    //Имя
    let tooltipName = "Иванов Иван";
    //Телефон
    let tooltipPhone = "+7 XXXXXXXXXX";

    //---------Phone mask---------//
    // Код страны
    let phoneCode = '+';
    // Максимальное количество цифр в номере (без учёта +)
    let MaxLengthPhoneNumber = 16;
    ...
    let mask = {
        ...
        init: function () {
            $(this.phoneInputs).attr("maxlength", (16 + 1));

            if (this.phoneInputs) {
                $(this.phoneInputs).attr('pattern','(\\+)[0-9]{11,16}');
